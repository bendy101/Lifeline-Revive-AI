...
Lifeline_check_available_medic = {
	params ["_unit","_incap"];
	    ...
	    && (if (Lifeline_ASmission) then {if (_unit == Petros) then {false} else {true}} else {true}) //check not antistasi commander
};
...
params ["_unit","_revivepos"];
	//check its right direction 
	_checkdegrees = [_revivepos,_medic,15] call Lifeline_checkdegrees;//
	if (_medic getVariable ["ReviveInProgress",0] == 2 && _checkdegrees == false) then {
